...
class TestCity(unittest.TestCase):
    ...
    def test_update_city(self):
        ...
        with patch('sys.stdout', new=StringIO()) as f:
            cty = City()
            cty.name = "Lagos"
            HBNBCommand().onecmd(f'update City {cty.id} name "Abuja"')
            self.assertEqual(cty.__dict__["name"], "Abuja")
...
class TestCityDotNotation(unittest.TestCase):
    ...
    def test_show_city(self):
        ...
        with patch('sys.stdout', new=StringIO()) as f:
            ...
            HBNBCommand().onecmd(HBNBCommand().precmd(
                                 f'City.show({city.id})'))
            res = f"[{type(city).__name__}] ({city.id}) {city.__dict__}"
            self.assertEqual(f.getvalue().strip(), res)

    def test_update_city(self):
        """Test to update city object.
        """
        with patch('sys.stdout', new=StringIO()) as f:
            city = City()
            city.name = "Lagos"
            HBNBCommand().onecmd(HBNBCommand().precmd(
                                 f'City.update({city.id}, name, "Abuja")'))
            self.assertEqual(city.__dict__["name"], "Abuja")

    def test_destroy_city(self):
        """Test to  destroy city object.
        """
        with patch('sys.stdout', new=StringIO()):
            city = City()
            HBNBCommand().onecmd(HBNBCommand().precmd(
                                 f'City.destroy({city.id})'))
            self.assertNotIn("City.{}".format(
                city.id), storage.all().keys())
